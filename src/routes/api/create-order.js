...
export const post = async ({request})=>{
    ...
    `('${element.labno}','${String(element.condition == '' ? '' : element.condition.map(values=>values.id))}','${element.collection_date}'::timestamp,'${element.collector}','${String(element.tests.map(values=>values.id))}','${orderId}','registrar','${element.sample_type}') `
    )
